...
def get_all_files_name_from_dir(path):
    """Get supported files from a directory.
    ...
    raw_or_mzxml_files = [
        ...
        if file.lower().endswith(('.raw', '.mzxml'))
    ]
    mzml_files = [
        file for file in files
    ...
    raw_or_mzxml_bases = {
        ...
        for file in raw_or_mzxml_files
    }
    mzml_bases = {
        os.path.splitext(file)[0].lower()
    ...
    if (
        raw_or_mzxml_files
        and len(raw_or_mzxml_files) == len(mzml_files)
        and raw_or_mzxml_bases == mzml_bases
    ):
        print("  Found matching mzML files for all RAW/mzXML files; using mzML files only.")
        files = mzml_files
...
def _unique_output_path(output_dir, filename):
    """Return a non-existing path in output_dir for filename."""
    output_abs_dir = os.path.abspath(os.fspath(output_dir))
    os.makedirs(output_abs_dir, exist_ok=True)
    target_path = os.path.join(output_abs_dir, filename)
...
def _convert_mzxml_to_mzml_docker(mzxml_file_path, output_dir, reuse_existing=True):
    """
    ...
    try:
        ...
        if os.path.exists(mzml_path):
            ...
            return mzml_path
        elif os.path.exists(existing_mzml_path):
            print(f"  ✓ Conversion output found at: {existing_mzml_path}")
            return existing_mzml_path
        else:
...
def _convert_raw_to_mzml_docker(raw_file_path, output_dir, reuse_existing=True):
    """
    ...
    existing_mzml_path = os.path.join(output_abs_dir, mzml_filename)
    if reuse_existing and os.path.exists(existing_mzml_path):
        print(f"  Found existing mzML file, reusing: {existing_mzml_path}")
        return existing_mzml_path

    mzml_path = _unique_output_path(output_abs_dir, mzml_filename)
    mzml_filename = os.path.basename(mzml_path)
    ...
    docker_cmd = [
        ...
        "-v", f"{output_abs_dir}:/output",
        "chambm/pwiz-skyline-i-agree-to-the-vendor-licenses",
        "wine", "msconvert", f"/input/{raw_filename}", "--mzML",
        "--outdir", "/output",
        "--outfile", mzml_filename
    ]
...
def _extract_rar_file(rar_path, extract_to):
    """Extract a rar file and return list of extracted files."""
    try:
        import rarfile
        ...
        extracted_files = _list_files_under(extract_to)
        print(f"  ✓ Extracted rar file to: {extract_to}")
        return extracted_files
    except ImportError:
        pass
    except Exception as e:
        print(f"  Warning: Python rarfile extraction failed: {e}")

    extractor_commands = []
    if shutil.which("unar"):
        extractor_commands.append(
            ["unar", "-quiet", "-force-overwrite", "-output-directory", extract_to, rar_path]
        )
    if shutil.which("unrar"):
        extractor_commands.append(["unrar", "x", "-y", rar_path, extract_to + os.sep])
    if shutil.which("7z"):
        extractor_commands.append(["7z", "x", "-y", f"-o{extract_to}", rar_path])
    if shutil.which("bsdtar"):
        extractor_commands.append(["bsdtar", "-xf", rar_path, "-C", extract_to])

    for command in extractor_commands:
        try:
            ...
            print(f"  Warning: RAR extractor failed ({command[0]}): {e}")
            if e.stderr:
                print(f"    stderr: {e.stderr.strip()}")

    print("  ✗ Failed to extract rar file. Install one of: unar, unrar, 7z, bsdtar, or Python rarfile with a RAR backend.")
    return []


def _copy_mzml_to_output(file_path, output_dir):
    """Copy an mzML file from a temporary extraction folder into the output directory."""
    file_abs_path = os.path.abspath(os.fspath(file_path))
    output_abs_dir = os.path.abspath(os.fspath(output_dir))
    os.makedirs(output_abs_dir, exist_ok=True)

    if os.path.dirname(file_abs_path) == output_abs_dir:
        return file_abs_path

    target_path = os.path.join(output_abs_dir, os.path.basename(file_abs_path))
    if os.path.exists(target_path):
        try:
            if os.path.samefile(file_abs_path, target_path):
                return target_path
        except OSError:
            pass

        target_path = _unique_output_path(output_abs_dir, os.path.basename(file_abs_path))

    shutil.copy2(file_abs_path, target_path)
    print(f"  Copied mzML to output directory: {target_path}")
    return target_path


def _process_downloaded_file(file_path, output_dir, reuse_existing_mzml=True):
    """
    ...
    if file_lower.endswith('.raw'):
        ...
        mzml_path = _convert_raw_to_mzml_docker(
            file_path,
            output_dir,
        ...
        mzml_path = _convert_mzxml_to_mzml_docker(
            file_path,
            output_dir,
            reuse_existing=reuse_existing_mzml
        )
        if mzml_path and os.path.exists(mzml_path):
            ...
            try:
                ...
                print(f"  Keep the original mzXML file")
            except:
                pass
        else:
            print(f"  ✗ Conversion failed")
        ...
        if os.path.exists(file_path):
            mzml_files.append(_copy_mzml_to_output(file_path, output_dir))
        else:
        ...
        if archive_type == "rar":
            extracted_files = _extract_rar_file(file_path, extract_dir)
        else:
...
def _match_mz(i, df_mz, df, ms_error=MASS_TOLERANCE, retention=False, implicit_single_charge=False):
    mz = df.precursor_mass.values.tolist()[i] if implicit_single_charge else df["m/z"].values.tolist()[i]
    masses = df_mz.Mass.values.tolist()
    idx = [k for k in range(len(masses)) if masses[k] - ms_error < mz < masses[k] + ms_error]
    if retention:
        rt = df.peak_RT.values.tolist()[i] if 'peak_RT' in df else df.RT.values.tolist()[i]
        mapping_rts = df_mz.RT.values.tolist()
        idx = [k for k in idx if mapping_rts[k] - RT_TOLERANCE < rt < mapping_rts[k] + RT_TOLERANCE]
    glycan_col = df_mz.glycan.values.tolist()
    return list(set([glycan_col[k] for k in idx]))
...
def _candidate_glycan_matches(i, df_mz, df, ms_error=MASS_TOLERANCE, retention=False,
    ...
    row = df.iloc[i]
    observed_mz = row['precursor_mass'] if implicit_single_charge else row['m/z']
    match_rt = _get_match_rt(row)
    xic_penalty = _xic_confidence_penalty(row)
    has_rt = retention and 'RT' in df_mz.columns
    masses = df_mz.Mass.values.tolist()
    glycans = df_mz.glycan.values.tolist()
    mapping_rts = df_mz.RT.values.tolist() if has_rt else None
    candidates = []
    for k in range(len(masses)):
        mass_error = abs(float(masses[k]) - float(observed_mz))
        if mass_error > ms_error:
            continue
        rt_error = 0.0
        if has_rt:
            mapping_rt = mapping_rts[k]
        ...
        score += xic_penalty
        candidates.append({'glycan': glycans[k], 'score': score, 'mass_error': mass_error, 'rt_error': rt_error})
    best_by_glycan = {}
...
def _glycofy_df(df, df_mz, filename, glycopost_id, ms_error=MASS_TOLERANCE, retention=False,
    ...
    meta_mod = str(mzml_metadata.get('modification') or '').lower()
    sample_prep = 'permethylated' if 'permethyl' in meta_mod else 'underivatized'
    modification = 'reduced' # TO-DO: THIS IS CURRENTLY HARDCODED AND NEEDS TO BE SET DURING EXTRACTION
    ...
    if xic_mode == "score":
        ...
        df['glycan'] = [_match_mz(k, df_mz, df, ms_error = ms_error,
                                  ...
                                  implicit_single_charge = implicit_single_charge)
                        for k in range(len(df))]
        glycan_lists = df.glycan.values.tolist()
        df['glycan_match_count'] = [len(set(matches)) for matches in glycan_lists]
        idx = [k for k in range(len(df)) if len(set(glycan_lists[k])) == 1]
        df_out = df.iloc[idx, :].reset_index(drop = True)
        df_out['glycan'] = [g[0] for g in df_out.glycan.values.tolist()]
        df_out = df_out.dropna(subset = ['glycan']).reset_index(drop = True)

    # Prefer CandyCrunch charge assignment, but keep mzML charges if that optional
    # prediction dependency is not importable in the active Python environment.
    try:
        from candycrunch.prediction import mass_check
    except Exception as exc:
        print(
            "    Warning: could not import candycrunch.prediction.mass_check; "
            "keeping precursor_charge values from the mzML file. "
            f"Original import error: {type(exc).__name__}: {exc}"
        )
    else:
        assigned_charges = []
        for observed_mz, glycan in zip(df_out['m/z'].values, df_out['glycan'].values):
            assigned = np.nan
            for z in PERMITTED_CHARGES:
                if mass_check(observed_mz, glycan, mode = ion_mode, mass_tolerance = ms_error,
                              permitted_charges = [z], sample_prep = sample_prep, modification = modification):
                    assigned = z
                    break
            assigned_charges.append(assigned)
        df_out['precursor_charge'] = assigned_charges
    ...
    if implicit_single_charge:
        df_out = df_out.drop(['precursor_mass'], axis=1)
    return df_out
...
def data_extraction(mzml_files, glycopost_id, df_mz_total, out_path, glycan_class=1, retention=True,
                    xic_mode="score", mass_tolerance=MASS_TOLERANCE, output_id=None):
    ...
    result_id = output_id or f"{glycopost_id}_{glycan_class}"
    dfs_train, df_train, df_pretrain = [], pd.DataFrame(), pd.DataFrame()
    unique_glycans = 0
    metadata_cols = {"glycan_class", "rt_group"}
    for k in mzml_files:
        df_mz = df_mz_total.copy()
        for j in [c for c in df_mz_total.columns.tolist()[3:] if c not in metadata_cols]:
            if j in k:
    ...
    if dfs_train:
        ...
        train_output = os.path.join(out_path, f'train_{result_id}.pkl')
        df_train.to_pickle(train_output)
    ...
    if dfs_pretrain:
        ...
        pretrain_output = os.path.join(out_path, f'pretrain_{result_id}.pkl')
        df_pretrain.to_pickle(pretrain_output)
    ...
    print(f"  Number of Pre-Training datapoints: {len(df_pretrain)}")
    return {
        "training_datapoints": len(df_train),
        "unique_glycans": unique_glycans,
        "pretraining_datapoints": len(df_pretrain),
    }
...
def main(args):
    ...
    output_dir = os.path.abspath(os.fspath(args.output_dir))
    mapping_file = args.mapping_file
    glycan_class = args.glycan_class
    retention = args.retention
    files_downloaded = args.files_downloaded
    files_path = os.path.abspath(os.fspath(args.files_path)) if args.files_path else args.files_path
    xic_mode = args.xic_mode
    group_run_id = getattr(args, "group_run_id", None)

    summary = {
        "training_datapoints": 0,
        "unique_glycans": 0,
        "pretraining_datapoints": 0,
    }
    mzml_files = []
    ...
    if isinstance(mapping_file, pd.DataFrame):
        has_mapping_data = not mapping_file.empty
    else:
        has_mapping_data = bool(mapping_file)

    if mzml_files and has_mapping_data:
        print("\n" + "=" * 60)
...
def build_arg_parser():
    """Build the command-line parser so it can also be reused after import."""
    ...
    parser.add_argument("--group-run-id", type=str, required=False,
                        help="Optional output identifier used in train/pretrain pickle filenames")
    return parser


def parse_args(argv=None):
    """Parse CLI-style arguments from a list, or from sys.argv if argv is None."""
...
def run_processing(
        ...
        xic_mz_tolerance=XIC_MZ_TOLERANCE,
        xic_score_margin=XIC_SCORE_MARGIN,
        group_run_id=None):
    """Run the full pipeline from imported Python code using the same options as the CLI."""
    if xic_mode not in XIC_MODES:
        raise ValueError(f"xic_mode must be one of {sorted(XIC_MODES)}, got {xic_mode!r}")

    args = argparse.Namespace(
        glcopost_id=glcopost_id,
        output_dir=output_dir,
        mapping_file=mapping_file,
        glycan_class=glycan_class,
        retention=_str_to_bool(retention),
        files_path=files_path,
        files_downloaded=_str_to_bool(files_downloaded),
        xic_mode=xic_mode,
        mass_tolerance=mass_tolerance,
        rt_tolerance=rt_tolerance,
        xic_mz_tolerance=xic_mz_tolerance,
